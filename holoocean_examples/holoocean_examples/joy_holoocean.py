...
class AgentController:
    ...
    def agent_command(self, axes: list[float],
        ...
        if self.type in ('BlueROV2', 'HoveringAUV'):
            max_thrust = 28.75
        ...
        elif self.type in ('TorpedoAUV', 'CougUV'):
            max_thrust = 1500
            max_delta  = 0.1
...
class JoyToAgentCommand(Node):
    def __init__(self):
        ...
        # Load agent names and types from the scenario JSON.
        ...
        if len(buttons) < len(names):
            self.get_logger().warn(
                f'agents.buttons has {len(buttons)} entries but scenario has {len(names)} agents. '
                'Extra agents will have no button assigned.'
            )
        ...
        for i, (name, atype, agent) in enumerate(zip(names, types, scenario_agents)):
            ...
            if i < len(buttons):
                ...
                self.get_logger().info(f'  agent [{name}]  type={atype}  button={buttons[i]}  camera={ctrl.has_camera}')
            else:
                self.get_logger().info(f'  agent [{name}]  type={atype}  (no button)  camera={ctrl.has_camera}')

        if default not in self.controllers:
            default = names[0]
        self.active: AgentController = self.controllers[default]

        self.enabled = True # Start enabled for convienience, but allow disarming with the button
    ...
    def _joy_cb(self, msg: Joy):
        ...
        # Vehicle command
        cmd = self.active.agent_command(axes, self.axis_map)
